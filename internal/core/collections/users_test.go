...
type userMatcher struct {
	expected core_models.User
}

func (m userMatcher) Matches(x interface{}) bool {
	user, ok := x.(core_models.User)
	if !ok {
		return false
	}
...
func TestNewUserCollection(t *testing.T) {
	...
	mockHandler := mongo_mocks.NewMockCollectionHandler[core_models.User](ctrl)
...
func TestUserCollection_GetUsersByTenantID(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			tenantID: "tenant1",
			returnUsers: []core_models.User{
				core_models.User{TenantID: "tenant1", Username: "user1"},
				core_models.User{TenantID: "tenant1", Username: "user2"},
			},
			expectedFilter: map[string]any{
				"tenant_id": "tenant1",
			},
...
func TestUserCollection_UpdateUser(t *testing.T) {
	...
	testCases := []struct {
		name                 string
		user                 core_models.User
		returnFindUser       *core_models.User
